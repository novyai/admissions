...
import { formatDuration, parseBasicAuthHeader } from "./utils"
...
export class Log {
  ...
  formatCommon(): string {
    ...
    const formattedDate: string = format(Date.now(), "dd/MMM/yyyy:HH:mm:ss xx")
  ...
  formatShort(): string {
    ...
    const queryString =
      Object.keys(this.logObject.request.url.params).length ?
