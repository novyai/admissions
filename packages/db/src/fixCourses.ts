...
import { LogicalOperator, RequirementType, db } from "@db/client"
    ...
    try {
      ...
      if (!courseInDb) {
        ...
        await db.prerequisite.deleteMany({
          where: {
            conditionId: {
              in: courseInDb.conditions.flatMap(condition => condition.conditions).map(condition => condition.id)
            }
          }
        })
        ...
        for (const conditionGroup of course.conditions) {
          ...
          for (const condition of conditionGroup.conditions) {
            ...
            for (const prerequisite of condition.prerequisites) {
              console.log(`Adding prerequisite ${prerequisite.courseSubject} ${prerequisite.courseNumber}`)
      ...
      console.error(`Error fixing course in DB: ${course.courseSubject} ${course.courseNumber}`, error);
    }
  }
}
